...
class LoaderExecuteInternalPlugin {
  ...
  apply(compiler) {
    compiler.hooks.normalModuleFactory.tap(pluginName, function (factory) {
      factory.hooks.parser.tap("javascript/auto", pluginName, function (parser, parserOptions) {
    ...
    compiler.hooks.compile.tap(pluginName, function (compilationParams) {// console.log(compiler);
    });
    compiler.hooks.compilation.tap(pluginName, function (compilation) {
      ...
      compilation.hooks.succeedEntry.tap(pluginName, function (entry, name, module) {
        console.log("compilation succeedEntry:", entry, name, module);
      });
      compilation.hooks.dependencyReference.tap(pluginName, function (dependencyReference, dependency, module) {
        console.log("compilation dependencyReference:", module);
      });
      compilation.hooks.finishModules.tap(pluginName, function (modules) {
...
var _default = LoaderExecuteInternalPlugin;
